...
"""
    ...
    def _check_existing_resources(self) -> Tuple[bool, bool, bool]:
        ...
        policy_arn = (
            self.config_manager.get("automated_reasoning_policy_arn")
            if self.config_manager
            else None
        )
        guardrail_id = (
            self.config_manager.get("automated_reasoning_guardrail_id")
            if self.config_manager
            else None
        )
        ...
        if guardrail_id:
            try:
                ...
                logger.info(
                    f"Guardrail check: {guardrail_id} (status: {guardrail_status})"
                )

                if not guardrail_exists:
                    logger.warning(
                        f"Guardrail exists but not ready: {guardrail_status}"
                    )
            except Exception as e:
                ...
                if self.config_manager:
                    ...
                    self.config_manager.set(
                        "automated_reasoning_guardrail_version", None
                    )
    ...
    def _create_policy(self) -> str:
        """Create or reuse Automated Reasoning policy (idempotent)"""

        # Use a consistent naming pattern for idempotency
        consistent_policy_name = "timecard-math-validation-policy"
        ...
        if existing_policy_arn and existing_policy_arn != "None":
            try:
                ...
                if policy_details.get("status") == "ACTIVE":
                    logger.info(
                        f"Using existing policy from database: {existing_policy_arn}"
                    )
                    return existing_policy_arn
                else:
                    logger.warning(
                        f"Policy in database not active: {policy_details.get('status')}"
                    )
            except Exception as e:
        ...
        # Check for existing policy with consistent name (idempotent approach)
        try:
            existing_policies = self.bedrock_client.list_automated_reasoning_policies()
            for policy in existing_policies.get("policies", []):
                policy_name = policy.get("name", "")

                # Look for our consistent name OR old naming pattern
                if policy_name == consistent_policy_name or policy_name.startswith(
                    "timecard-math-validation-"
                ):
                    ...
                    try:
                        policy_details = (
                            self.bedrock_client.get_automated_reasoning_policy(
                                policyArn=policy_arn
                            )
                        )
                        if policy_details.get("status") == "ACTIVE":
                            logger.info(
                                f"Found existing active policy: {policy_name} ({policy_arn})"
                            )
                            ...
                            logger.info(
                                f"Found policy but not active: {policy_name} (status: {policy_status})"
                            )
                    except Exception as verify_e:
                        logger.warning(
                            f"Policy exists in list but not accessible: {verify_e}"
                        )
                        continue
        except Exception as e:
            logger.warning(f"Failed to check existing policies: {e}")

        # Create new policy with consistent name (idempotent)
        ...
        try:
            response = self.bedrock_client.create_automated_reasoning_policy(
                name=consistent_policy_name,
                description=description,
                clientRequestToken=str(uuid.uuid4()),
            )

            policy_arn = response["policyArn"]
            logger.info(f"Created new policy: {consistent_policy_name} ({policy_arn})")
            ...
            # Check if it failed because policy with same name already exists
            if "already exists" in str(e).lower() or "duplicate" in str(e).lower():
                logger.info("Policy with same name already exists, searching for it...")
                # Try to find it again
                try:
                    existing_policies = (
                        self.bedrock_client.list_automated_reasoning_policies()
                    )
                    for policy in existing_policies.get("policies", []):
                        if policy.get("name") == consistent_policy_name:
                            policy_arn = policy.get("policyArn")
                            logger.info(
                                f"Found existing policy after creation conflict: {policy_arn}"
                            )
                            if self.config_manager:
                                self.config_manager.set(
                                    "automated_reasoning_policy_arn", policy_arn
                    ...
                    logger.error(
                        f"Failed to find existing policy after conflict: {search_e}"
                    )
    ...
    def _upload_policy_document(self, policy_arn: str) -> str:
        ...
        try:
            ...
            response = (
                self.bedrock_client.start_automated_reasoning_policy_build_workflow(
                    policyArn=policy_arn,
    ...
    def _wait_for_policy_ready(
        self, policy_arn: str, build_workflow_id: str, max_wait: int = 300
    ) -> bool:
        ...
        while time.time() - start_time < max_wait:
            try:
                ...
                build_response = (
                    self.bedrock_client.get_automated_reasoning_policy_build_workflow(
                        policyArn=policy_arn, buildWorkflowId=build_workflow_id
                    )
                )
    ...
    def _create_guardrail(self, policy_arn: str) -> Tuple[str, str]:
        """Create or reuse guardrail with the policy attached (idempotent)"""

        # Use a consistent naming pattern for idempotency
        consistent_guardrail_name = "timecard-math-guardrail"
        ...
        if existing_guardrail_id:
            try:
                ...
                if guardrail_details.get("status") == "READY":
                    ...
                    ar_config = guardrail_details.get(
                        "automatedReasoningPolicyConfig", {}
                    )
                    policies = ar_config.get("policies", [])
                    if policy_arn in policies:
                        logger.info(
                            f"Using existing guardrail from database: {existing_guardrail_id}"
                        )
                        return existing_guardrail_id, guardrail_details.get(
                            "version", "DRAFT"
                        )
                    else:
                        logger.warning(
                            f"Guardrail in database has different policy attached"
                        )
                        logger.info(f"Expected policy: {policy_arn}")
                        logger.info(f"Found policies: {policies}")
                        # Try to update the guardrail to use the correct policy
                        try:
                            logger.info(
                                "Attempting to update guardrail with correct policy..."
                            )
                            update_response = self.bedrock_client.update_guardrail(
                                guardrailIdentifier=existing_guardrail_id,
                                name=guardrail_details.get(
                                    "name", consistent_guardrail_name
                                ),
                                description=guardrail_details.get(
                                    "description",
                                    "Mathematical validation guardrail for timecard data",
                                ),
                                automatedReasoningPolicyConfig={
                                    "policies": [policy_arn],
                            ...
                            logger.info(
                                f"Successfully updated guardrail with correct policy"
                            )
                            return existing_guardrail_id, update_response.get(
                                "version", "DRAFT"
                            )
                        except Exception as update_e:
                            logger.warning(f"Failed to update guardrail: {update_e}")
                else:
                    logger.warning(
                        f"Guardrail in database not ready: {guardrail_details.get('status')}"
                    )
            except Exception as e:
        ...
        # Check for existing guardrail with consistent name or old naming pattern
        try:
            ...
            for guardrail in existing_guardrails.get("guardrails", []):
                ...
                # Look for our consistent name OR old naming pattern
                if (
                    guardrail_name == consistent_guardrail_name
                    or guardrail_name.startswith("timecard-math-guardrail-")
                ):
                    ...
                    try:
                        ...
                        if guardrail_details.get("status") == "READY":
                            ...
                            if policy_arn in policies:
                                logger.info(
                                    f"Found existing compatible guardrail: {guardrail_name} ({guardrail_id})"
                                )
                                # Save to database for future use
                                if self.config_manager:
                                    self.config_manager.set(
                                ...
                                return guardrail_id, guardrail_details.get(
                                    "version", "DRAFT"
                                )
                            else:
                                logger.info(
                                    f"Found guardrail but with different policy, attempting to update: {guardrail_name}"
                                )
                                # Try to update it with the correct policy
                                try:
                                    update_response = self.bedrock_client.update_guardrail(
                                        guardrailIdentifier=guardrail_id,
                                        name=guardrail_name,
                                    ...
                                    logger.info(
                                        f"Successfully updated existing guardrail with correct policy"
                                    )
                                    if self.config_manager:
                                        self.config_manager.set(
                                    ...
                                    return guardrail_id, update_response.get(
                                        "version", "DRAFT"
                                    )
                                except Exception as update_e:
                                    logger.warning(
                                        f"Failed to update existing guardrail: {update_e}"
                                    )
                        else:
                            logger.info(
                                f"Found guardrail but not ready: {guardrail_name} (status: {guardrail_status})"
                            )
                    except Exception as verify_e:
                        logger.warning(
                            f"Guardrail exists in list but not accessible: {verify_e}"
                        )
                        continue
        except Exception as e:
            logger.warning(f"Failed to check existing guardrails: {e}")

        # Create new guardrail with consistent name (idempotent)
        try:
            ...
            logger.info(
                f"Created guardrail: {consistent_guardrail_name} ({guardrail_id})"
            )
            ...
            # Check if it failed because guardrail with same name already exists
            if "already exists" in str(e).lower() or "duplicate" in str(e).lower():
                logger.info(
                    "Guardrail with same name already exists, searching for it..."
                )
                # Try to find it again
                try:
                    existing_guardrails = self.bedrock_client.list_guardrails()
                    for guardrail in existing_guardrails.get("guardrails", []):
                        if guardrail.get("name") == consistent_guardrail_name:
                            guardrail_id = guardrail.get("id")
                            logger.info(
                                f"Found existing guardrail after creation conflict: {guardrail_id}"
                            )

                            # Get details and check policy
                            guardrail_details = self.bedrock_client.get_guardrail(
                                guardrailIdentifier=guardrail_id
                            )
                            ...
                            if self.config_manager:
                                ...
                                self.config_manager.set(
                                    ...
                                    guardrail_details.get("version", "DRAFT"),
                                )
                    ...
                    logger.error(
                        f"Failed to find existing guardrail after conflict: {search_e}"
                    )
    ...
    def _test_guardrail(self, guardrail_id: str, guardrail_version: str) -> bool:
        ...
        try:
            ...
            logger.info(
                f"Guardrail test results: {tests_passed}/3 tests passed ({success_rate:.1%})"
            )
    ...
    def _update_config(
        self, policy_arn: str, guardrail_id: str, guardrail_version: str
    ):
        ...
        if self.config_manager:
            try:
                ...
                logger.info(
                    "Updated configuration with new Automated Reasoning settings"
                )
            except Exception as e:
    ...
    def ensure_provisioned(self, force_recreate: bool = False) -> Dict[str, Any]:
        ...
        try:
            ...
            if not lock_acquired:
                logger.info(
                    "Another instance is provisioning, checking current status..."
                )
            ...
            try:
                ...
                build_workflow_id = (
                    self.config_manager.get("automated_reasoning_build_workflow_id")
                    if self.config_manager
                    else None
                )
                ...
                if current_status == "ready" and not force_recreate:
                    ...
                    try:
                        ...
                        if not policy_arn or not guardrail_id:
                            logger.warning(
                                "Missing policy ARN or guardrail ID in database"
                            )
                            # Before starting fresh, check if there are existing resources we can use
                            policy_exists, guardrail_exists, properly_connected = (
                                self._check_existing_resources()
                            )
                            if policy_exists or guardrail_exists:
                                logger.info(
                                    "Found existing resources, attempting to use them..."
                                )
                                return self._get_current_status()
                            logger.info(
                                "No existing resources found, starting fresh provisioning..."
                            )
                        ...
                        try:
                            ...
                            if policy_status != "ACTIVE":
                                logger.warning(
                                    f"Policy exists but not active: {policy_status}"
                                )
                                return self._start_async_creation()
                        except Exception as e:
                        ...
                        ar_config = guardrail_response.get(
                        ...
                        logger.info(
                            "Automated Reasoning READY - All resources verified"
                        )
                        logger.info(
                            f"   Policy: {policy_arn} (status: {policy_status})"
                        )
                        logger.info(
                            f"   Guardrail: {guardrail_id} (status: {guardrail_status})"
                        )
                        logger.info(f"   Policy properly attached to guardrail")
                        ...
                        return {
                            ...
                            "guardrail_version": self.config_manager.get(
                        ...
                        logger.info(
                            "Starting fresh provisioning due to verification failure..."
                        )
                        return self._start_async_creation()
                ...
                if current_status in ["not_configured", "failed"] or force_recreate:
                    logger.info(
                        f"Starting Automated Reasoning creation (reason: {current_status})"
                    )
                    if force_recreate:
    ...
    def _start_async_creation(self) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.info(
                f"Existing resources - Policy: {existing_policy_arn}, Guardrail: {existing_guardrail_id}, Workflow: {existing_build_workflow_id}"
            )

            # If we have an ongoing build workflow, don't create new resources
            if existing_policy_arn and existing_build_workflow_id:
                logger.info(
                    "Found existing policy and build workflow, checking status..."
                )
                try:
                    ...
                    if build_status in ["BUILDING", "QUEUED"]:
                        logger.info(
                            "Build workflow still in progress, not creating new resources"
                        )
                        return {
            ...
            if existing_policy_arn or existing_guardrail_id:
                ...
                if policy_exists and not guardrail_exists:
                    ...
                    try:
                        ...
                        guardrail_id, guardrail_version = self._create_guardrail(
                            policy_arn
                        )
    ...
    def _check_creation_progress(
        self, policy_arn: str, build_workflow_id: str
    ) -> Dict[str, Any]:
            ...
            if build_status == "COMPLETED":
                ...
                try:
                    ...
                    if self.config_manager:
                        ...
                        logger.info(
                            f"Updating DB to ready status with guardrail: {guardrail_id}"
                        )
                        if hasattr(self.config_manager, "update_multiple"):
                            self.config_manager.update_multiple(ready_settings)
                        else:
    ...
    def cleanup_orphaned_resources(self) -> Dict[str, Any]:
        """Clean up orphaned resources that are not tracked in database"""
        cleanup_results = {
            "policies_found": 0,
            "policies_cleaned": 0,
            "guardrails_found": 0,
            "guardrails_cleaned": 0,
            "errors": [],
        }
        ...
        try:
            ...
            tracked_guardrail_id = (
                self.config_manager.get("automated_reasoning_guardrail_id")
                if self.config_manager
                else None
            )

            logger.info(f"Cleanup: Tracked policy: {tracked_policy_arn}")
            logger.info(f"Cleanup: Tracked guardrail: {tracked_guardrail_id}")
            ...
            try:
                ...
                for policy in existing_policies.get("policies", []):
                    ...
                    if (
                        policy_name.startswith("timecard-math-validation-")
                        and policy_name != "timecard-math-validation-policy"
                    ):
                        cleanup_results["policies_found"] += 1

                        # Only clean up if it's not the tracked one
                        if policy_arn != tracked_policy_arn:
                            try:
                                logger.info(
                                    f"Cleaning up orphaned policy: {policy_name} ({policy_arn})"
                                )
                                # Note: AWS doesn't allow deleting policies directly, they expire automatically
                                # We just log them for manual cleanup if needed
                                logger.warning(
                                    f"Found orphaned policy (manual cleanup may be needed): {policy_name}"
                                )
                            except Exception as e:
                                cleanup_results["errors"].append(
                                    f"Failed to clean policy {policy_name}: {e}"
                                )
                        else:
                            logger.info(f"Keeping tracked policy: {policy_name}")
            except Exception as e:
                cleanup_results["errors"].append(f"Failed to list policies: {e}")

            # Clean up orphaned guardrails
            try:
                existing_guardrails = self.bedrock_client.list_guardrails()
                for guardrail in existing_guardrails.get("guardrails", []):
                    guardrail_name = guardrail.get("name", "")
                    ...
                    if (
                        guardrail_name.startswith("timecard-math-guardrail-")
                        and guardrail_name != "timecard-math-guardrail"
                    ):
                        cleanup_results["guardrails_found"] += 1

                        # Only clean up if it's not the tracked one
                        if guardrail_id != tracked_guardrail_id:
                            try:
                                logger.info(
                                    f"Cleaning up orphaned guardrail: {guardrail_name} ({guardrail_id})"
                                )
                                self.bedrock_client.delete_guardrail(
                                    guardrailIdentifier=guardrail_id
                                )
                                cleanup_results["guardrails_cleaned"] += 1
                                logger.info(
                                    f"Successfully deleted orphaned guardrail: {guardrail_name}"
                                )
                            except Exception as e:
                                cleanup_results["errors"].append(
                                    f"Failed to delete guardrail {guardrail_name}: {e}"
                                )
                        else:
                            logger.info(f"Keeping tracked guardrail: {guardrail_name}")
            except Exception as e:
                cleanup_results["errors"].append(f"Failed to list guardrails: {e}")
    ...
    def _get_current_status_with_smart_check(self) -> Dict[str, Any]:
        ...
        current_status = self.config_manager.get(
            "automated_reasoning_status", "not_configured"
        )
        ...
        if current_status == "creating":
            build_workflow_id = self.config_manager.get(
                "automated_reasoning_build_workflow_id"
            )
    ...
    def get_current_config(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "status": self.config_manager.get(
                "automated_reasoning_status", "not_configured"
            ),
        }
...
def auto_provision_if_needed(
    config_manager=None, region_name: str = "us-west-2"
) -> Dict[str, Any]:
    """
